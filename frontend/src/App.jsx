...
import { Box, AppBar, Toolbar, Typography, IconButton, Drawer, List, ListItem, ListItemIcon, ListItemText, Avatar } from '@mui/material';
    ...
    switch (role) {
      ...
      case 'Admin':
        drawerItems = [
          ...
          { text: 'Support Queries', icon: <ForumIcon />, path: '/admin/queries' },
      ...
      {user && (
        <Box sx={{ p: 2, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Avatar sx={{ bgcolor: '#2979ff', width: 36, height: 36, flexShrink: 0 }}>{(user.name || user.Name)?.charAt(0) || 'U'}</Avatar>
            <Box sx={{ flexGrow: 1, minWidth: 0 }}>
              <Typography variant="subtitle2" sx={{ color: '#fff', fontWeight: 'bold', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.name || user.Name}</Typography>
              <Typography variant="caption" sx={{ color: '#a0aec0' }}>{t(`sidebar.${getRoleKey(user.role || user.Role)}`)}</Typography>
            </Box>
